...
### 2026-07-08 — First full run: bugs found under real conditions, fixed and backfilled

The first full democracy deploy exposed several issues invisible in the smoke/tuning runs (which omitted the `document` output group and used only glmnet/xgboost). All fixed; details in commit messages. Summary:

- **Document group aborts a fresh run.** The `tar_quarto` targets read the store via `tar_read_raw(<string>)`, which tarchetypes can't register as a dependency, so they render at pipeline *start* (before inputs exist) and `Appendix`'s failure — no `error="null"` — killed the whole run at 52s. Fix: dropped `document` from `full_democracy.yml` (render docs locally; **W1 follow-up: give the quarto targets explicit deps so they render last**), `error="null"` on all three quarto targets, and self-contained `has_tuning` chunk in Appendix_Methods.qmd.
- **EF-cache attrition root-caused (supersedes shakedown #11).** Parallel nodes DO have outbound network. The 23 missing volumes were a hathiTools 0.2.0 encoding bug (dotted miun/miua ids) + no rsync in the container. Fixed both (see the EF-cache note above). Full run: 0 attrition.
- **Performance-eval bugs for naivebayes/LiblineaR** (first run at scale): LiblineaR eval matrix not matched to training features (112× "columns differ"); naivebayes `as.matrix()` densifying huge OOD DFMs (out-of-memory, 8–35 GB); NULL model breaking `UseMethod` (7×). Fixed: LiblineaR `dfm_match`, naivebayes sparse `dgCMatrix` eval, `model_performance.NULL`, OOD block routed to bigmem. Backfilled from the store (resources/logic fix; cached science preserved — 9,696 skipped on redeploy).
- **kl/entropy/graph were `deployment="main"`** → serialized on the coordinator, and n=5 reps made this the dominant tail cost (workers idle while the coordinator single-threaded hundreds of branches). Changed to `deployment="worker"` + `storage/retrieval="worker"` so the pool parallelizes them. Non-invalidating (not part of the target hash; manifest byte-identical). **Applies to the next run** (current backfill pinned to the prior commit).

**Ops lesson:** the "0 workers + N dispatched" pattern is usually the coordinator busy on `deployment="main"` targets (kl/entropy/graph/combined tables), not a stall — check whether `completed` is advancing and the log mtime is fresh before concluding a hang.
...
### 2026-07-10 — Sample-diversity analysis; W4 done; KL prototype (W5c) done; store-resolution root cause found

**Sample-diversity / "do we need a 1000–2000-vol run?" analysis** (from `sampled_htids` + `rank_agreements`, decision support for Xavier's question):

- The 5×500 headline reps are **nearly disjoint** post-1820 (mean pairwise overlap 0.5–25 vols/500; union 2,100–2,500 distinct volumes/decade), so the run already reads ~2,500 distinct volumes per decade. Pre-1780 the pool is exhausted (78–504 volumes exist; all reps identical — repeats vary only split/fit noise there, and a larger cap changes nothing).
- **New finding — self-stability across reps is LOWER than cross-model agreement for the sparse linear engines** (post-1820 medians, Spearman / top-100 overlap): cross-engine-same-rep 0.23 / 0.35, vs LiblineaR-vs-itself-across-reps 0.06 / 0.19, glmnet 0.14 / 0.20; naivebayes 0.28 / 0.64 and xgboost 0.41 / 0.50 are moderately self-stable. At 500 vols/fit, sampling noise ≥ model disagreement for glmnet/LiblineaR — partly intrinsic lasso instability among correlated features, partly possible data starvation.
- **Decision guidance:** aggregate claims (entropy trends, agreement curves, rep-stable ism terms) are already settled by 5×500; single-model term-ranking claims are not, at any affordable n. Proposed (not yet run): a **stability run** `stability_democracy_{1790,1860,1940}.yml` — cap 1000, `n_repeats: 3`, 4 engines, weights-only outputs (tuning-study footprint, ~20–30 min/run on Rāpoi) — to test whether glmnet self-agreement rises materially at 1000 vols. Full-scale 1000-vol campaign only if it does. **JSTOR democracy run: parked** (different corpus serving the authority paper; cheap `dfr_audit` first step exists if revisited after W1–W5).

**W4 ✅ done (delegated to Opus, reviewed, one round of repair):** `graph_doc:` config block (reps/pos/weight_types/engines/periods; deep-merges corpus→run; `graph_doc: all` = full-QA opt-in) filters the browser doc's chunk list inside `write_run_graph_qmd()` — never the target grid, so no science invalidation (verified: manifest 849 rows, only `run_graph_qmd`'s command changed). `full_democracy` now renders rep-1 adjectives+ism predictive-weight graphs only: **175 → 8 graph chunks**. `render_quarto_file()` gained a post-render existence+mtime-freshness assertion (the "success without a file" bug now fails loudly).

**Store-resolution root cause found during review (supersedes the 2026-07-08 "suspects" list):** quarto executes chunks with the **document's directory (`Paper/`) as cwd**, and `execute-dir: project` in front matter is **silently ignored for standalone renders** (verified empirically). So a relative `_targets` store — and the old `here::here` + `custom.yaml` pattern, whose `tar_config_set(config = "custom.yaml")` never affects subsequent bare `tar_read` calls anyway — resolves to the nonexistent `Paper/_targets`. Fix shipped in the generator: setup chunk resolves `TARGETS_STORE` env → `_targets` if present in cwd → `../_targets`, normalizes, and `stop()`s if missing; **every** emitted `tar_read_raw()` carries explicit `store = targets_store`; no `tar_config_set` (no stray `Paper/_targets.yaml`); `execute-dir` line dropped. Suite **221 pass / 0 fail** (independently re-run). Not yet committed.

**W5c ✅ KL prototype done (delegated to Sonnet, reviewed):** `Paper/prototype_kl.qmd` (+ rendered HTML). `combined_kl` natively carries `measure ∈ {novelty, transience, resonance}` and clean engine labels (no lut repair needed). Figures: adjacent-decade novelty per model (mean ± rep range; free y-axes — KL levels not comparable across engines), full 32×32 past×future KL heatmap (glmnet rep 1), Barron-style resonance line + novelty-vs-transience scatter. Notable: naivebayes level-shift at 1830–40 (~10–16 → ~7–9 bits, sharpest structural break of any engine); xgboost's novelty curve uncorrelated with the others (r ≈ 0) while LiblineaR↔naivebayes r = 0.96; resonance is second-order (rarely ±3 bits vs 12–19 bit novelty baseline); 1710–1750 spikes flagged as corpus-scarcity noise. Units verified: KL/JSD in bits (`unit = "log2"`), entropy in nats.

**hathiTools upstream fix ✅ done, pushed, lockfile bumped (delegated to Opus, reviewed):** the pairtree-encoding bug is fixed in hathiTools 0.2.1 — `stubbytree()` gained an `encoder` argument (default `id_clean`, so the local cache layout is byte-identical) and the two remote-path sites (`stubby_url_to_rsync()`, plus the same bug found in the `download_http()` HTTP fallback) now use `id_encode()` via a new `id_encode_local()` helper; `local_loc()` deliberately unchanged so `find_cached_htids()` keeps working. 9 new regression tests (dotted/ark/plain ids + cache-path invariance, parity-verified against the Demagogues `ef_remote_path()` reference); package full suite 88 pass with all 30 fails/7 errors **pre-existing** (2022-era tibble/pillar snapshot drift + one rsync socket error unrelated to paths). Committed surgically as `xmarquez/hathiTools@c137626` (pre-existing WIP in that repo — untracked `htid_to_dfm.R`/`srp.R`, their NAMESPACE export and DESCRIPTION imports, regenerated man pages, snapshot artifacts — left uncommitted), pushed to master; verified in the Demagogues project library (0.2.1, dotted id → comma path) and pinned in renv.lock (commit `3c0b9f2`). The `rsync_ef_comma_encoded()` workaround in `cache_ef_files()` is now **redundant but kept as a safety net**; retire it after one clean cluster run on the rebuilt container. ⚠️ **Before the next cluster run:** the renv.lock push triggers the GHCR container rebuild — run `slurm/pull_image.sh` on Rāpoi once Actions finishes.

**New/remaining open items found during this review:**
- **Latent store bug elsewhere:** `R/jstor_corpus_functions.R:1364` and `R/latin_perseus_functions.R:1016` emit the same broken `tar_config_set(store = here::here("_targets"), config = "custom.yaml")` line into their generated docs; `Paper/Appendix.qmd` / `Appendix_Methods.qmd` use the same pattern with bare `tar_read_raw()` (they render locally only because of how they're invoked — this is very likely also why the in-pipeline `Appendix` target failed on the cluster, see W3). Fix the appendices as part of W3 using the W4 pattern (`graph_doc_store_setup_lines()` in `R/code_generation.R` is the reference); jstor/latin generators when those strands are next touched.
- `rmd_blocks_graph_names()` (legacy `graph_appendix.rmd` fragment for the `graph_document` target) still emits bare `tar_read()` with no store setup — retire or fix when `graph_document` is next used.
- Everything from this session is **uncommitted**: W4 change set (`R/code_generation.R`, `_targets.R`, `config/corpus.yml`, `config/runs/full_democracy.yml`, tests), `Paper/prototype_kl.qmd`, `Paper/prototype_ism_clusters.qmd` (untracked), this plan update. Commit after review. (The 2026-07-08 `canonical_weight_id()` fix landed as `0f3eef7`, despite the "not yet committed" note in that entry.)
...
### W4. Harden `run_graph_html` (graph browser doc) — ✅ DONE 2026-07-10 (see Progress log; uncommitted)
...
### W5. Visualize the cross-model similarity + information-theoretic measures — 🔶 PROTOTYPES DONE (2026-07-08/10); pipeline/paper integration pending

> All three figure families now exist as reviewed prototypes reading the local bundle: `Paper/prototype_measures.qmd` (similarity + entropy, 2026-07-08), `Paper/prototype_ism_clusters.qmd` (ism-term stability across models/reps, 2026-07-09), `Paper/prototype_kl.qmd` (KL novelty/transience/resonance, 2026-07-10). Remaining: pick the surviving panels, promote them into `Appendix.qmd`/the paper (W1/W3), fix the ism prototype's peak-period row ordering (weighted mean misleads for bimodal terms — use max/first-peak), and drop `prototype_measures.qmd`'s entropy-lut workaround once a new run refreshes the similarity targets with populated engine labels.
...
- **KL figures** (`combined_kl`, 527,310 rows: `P`/`past_period`/`future_period`/`measure`/`value`, 5 reps). **✅ Prototyped 2026-07-10** in `Paper/prototype_kl.qmd` (see Progress log): adjacent-decade novelty lines per model, past×future heatmap, Barron-style resonance. `measure` natively encodes `novelty`/`transience`/`resonance`; KL is in bits (`unit="log2"`). Pipeline/paper integration pending.
