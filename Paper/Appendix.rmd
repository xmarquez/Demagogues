...
archy_acy <- all_model_weights %>% 
  filter(str_detect(word, "archy_|cracy_"), !str_detect(source, "_random|_similarity")) %>% 
...
earliest_latest <- all_model_weights %>% 
  filter(!str_detect(source, "_random|_similarity"), scaled_value > 5) %>% 
  mutate(word = str_to_lower(word)) %>%
  group_by(word) %>%
...
model_graph <- function(term_list) {
  all_model_weights %>%
    mutate(word = str_to_lower(word)) %>%
    filter(word %in% term_list, !str_detect(source, "_random|_similarity")) %>%
    ggplot(aes(x = decade, y = pnormed_value)) +
